...
from utils.cli import parse_list_arg, setup_basic_parser, validate_args
...
def get_file_sizes(
    ...
    for file_path in find_files(
        directory, exclude_dirs=exclude_dirs, include_dirs=include_dirs
    ):
        # Skip excluded extensions
        if exclude_extensions and any(
            file_path.endswith(ext) for ext in exclude_extensions
        ):
            continue
...
def main() -> None:
    ...
    parser.add_argument(
        "--exclude", help="Comma-separated list of directories or extensions to exclude"
    )
    parser.add_argument(
        "--include", help="Comma-separated list of directories to include"
    )
    parser.add_argument(
        "--limit", type=int, default=25, help="Number of files to display (default: 25)"
    )
